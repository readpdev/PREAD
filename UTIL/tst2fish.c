...
char CompilerName[8]=
	...
	#pragma aux ReadTimeStampCounter = " db 0Fh,031h" value [eax] modify exact [eax
#endif
...
void TimeOps(int iterCnt)
	...
	for (n=0;n<TEST_CNT;n++)			/* gather all data into testTime[][] */
		{
		for (t.keySize=KEY_BITS_0,q=0;t.keySize<=MAX_KEY_BITS;t.keySize+=STEP_KEY_BITS
			{
			...
			for (k=0,minT=~0lu;k<4;k++)
				...
				switch (n)
					...
					case 2:
						...
						GetTimer();
						t0=GetTimer();
						CALL_N { t.ki.key32[0]+=0x87654321;	/* change key bytes to force cache mis
...
void AES_Test_ECB_D_MCT(CONST char *fname)
	...
	for (t.keySize=KEY_BITS_0,q=0;t.keySize <= MAX_KEY_BITS;t.keySize+=STEP_KEY_BIT
		{
		AES_printf(&t,"S\n");			/* output key size */
		if (!quietVerify) printf("  keyLen = %3d. ",t.keySize);
...
/*      moduleName,modeString,TwofishCodeSize(),*/
	...
	printf("%s MCT Generation : %d,%d.\n",
		   ((MCT_INNER == mctInner) && (MCT_OUTER == mctOuter)) ? "Full" : " *** Parti
		   mctOuter,mctInner);
