     ...
     d curusr                254    263
      ...
      /free
       ...
       cmd = 'ftpjob/ftpjob server(' + sq + '192.168.89.130' + sq +
       ...
       if sqlcod = 0 and count = 0;
         ...
         for x = 1 to 5;
           ...
           cmd = 'inztap dev(tapvrt01) newvol(' + volNam + ') vol(' + volNam +
             ') check(*no)';
           run(cmd:%len(%trim(cmd)));
           cmd = 'lodimgclge imgclg(backup) imgclgidx(*vol) option(*load) ' +
             'vol(' + volNam + ')';
           run(cmd:%len(%trim(cmd)));
           cmd = 'chgimgclge imgclg(backup) imgclgidx(*vol) vol(' + volNam +
             ') alcstg(*min)';
           run(cmd:%len(%trim(cmd)));
         endfor;
       else;
