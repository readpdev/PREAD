      *%METADATA                                                       *
      * %TEXT KEYCODEWEB version 01000015 as of 160622 @ 125858        *
      *%EMETADATA                                                      *
...
J6279d emlInst         s             70    dim(3) ctdata
     ...
     d handleA         s             24    dim(50)
     d                 ds
J6216d elements                     100    dim(57) ctdata
     d  elementID                    10    overlay(elements)
     d  elementTxt                   90    overlay(elements:*next)

     d upTo1stBlank    pr              *
     d  stringVal                      *   const options(*string:*trim)
...
J6279d run             pr            10i 0 extproc('system')
      ...
      * Create the XML license file.
J6279d crtLicFilXML    pr            10i 0

      * Create the TXT license file.
J6279d crtLicFilTXT    pr            10i 0

J6279d writeTxtBuf     pr
J6279d  handle                       10i 0 const
J6279d  buffer                      512    const

J6279D xmlHandler      pr            10i 0
J6279d  xmlOper                      10i 0 const
J6279d  xmlTag                         *   value
J6279d                                     options(*string:*trim:*nopass)
J6279d  xmlText                        *   const
J6279d                                     options(*string:*trim:*nopass)

J6279d convertCCSID    pr
J6279d operation                      5i 0 value
J6279d ioBuffer                        *   value options(*nopass)
J6279d ioBufferLen                   10i 0 value options(*nopass)
J6279d fromCCSID                     10i 0 value options(*nopass)
J6279d toCCSID                       10i 0 value options(*nopass)
      ...
      * Append data to buffer for output.
     d apnd            pr
     d  val1                        100    value
     d  val2                        100    options(*nopass) value
     d  val3                        100    options(*nopass) value
     d  val4                        100    options(*nopass) value
     d  val5                        100    options(*nopass) value
     d  val6                        100    options(*nopass) value
     d  val7                        100    options(*nopass) value
     d  val8                        100    options(*nopass) value
     d  val9                        100    options(*nopass) value
     d  val10                       100    options(*nopass) value
     d  val11                       100    options(*nopass) value
     d  val12                       100    options(*nopass) value
     d  val13                       100    options(*nopass) value
     d  val14                       100    options(*nopass) value
     d  val15                       100    options(*nopass) value
...
J6475    if skmver >= 902 and (skmpfm = '400' or skmpfm = 'DMWIN');
         ...
         // Attach the appropriate licnese files.
...
J6475    if skmver >= 902 and (skmpfm = '400' or skmpfm = 'DMWIN');
       ...
       begsr chkCurHandle;
         ...
         if curHandle = ' ' or %lookup(curHandle:handleA) = 0;
           exsr logOut;
         endif;
       endsr;
       //***********************************************************************
...
J6122      endif;
           ...
           apnd('<td>':%triml(%editw(skmver:'0 .  ')):'</td></tr>':lf);
T4550      apnd('<input type=hidden name=sid':%trim(%char(ordSeq)):
/            ' value=':%trim(skmsid):'>');
/          apnd('<input type=hidden name=isn':%trim(%char(ordSeq)):
/            ' value=':%trim(%char(skmis#)):'>':lf);
/          apnd('<input type=hidden name=pfm':%trim(%char(ordSeq)):
/            ' value=':%trim(skmpfm):'>':lf);
           exec sql fetch c1 into :mstdta;
           if ordSeq > 100;
         ...
         dow str > 0;
           ...
           if skpkca <> ' ';
             if prd# = 12;
               ...
               tiffAuth(skmsid:skmpfm:skmis#:skldmo:sktsvr);
             endif;
           endif;
...
T5805    open spykeyver;
       ...
       begsr logIn;
         ...
         apnd('<tr><td>':getElem('signOn'):'</td></tr>':lf);
       endsr;
       //***********************************************************************
J6295  begsr showHistory;
       ...
       begsr showCust;
         ...
         apnd('<table><tr><td>':getElem('logOut'):'</td><td>':getElem('search'):
           '</td><td>':getElem('history'):'</td></tr></table>':lf);
         apnd(getElem('custInfCap'):lf);
         apnd(getElem('sysIdOut'):skmsid:'</td></tr>':lf);
...
T6896      endif;
         ...
         apnd('<br>':lf);
         apnd(getElem('keyCode'):ko_p1+ko_p2+ko_p3+ko_p4:lf);          //T4686
         if skmver < 0700 and skmpfm = '400';
...
T5671    mailto = %trim(getElem('mailKey')) +
/          'Product key code.&body=Key Code: ' + keyout + #0A +
/          'Platform: ' + skmpfm + #0A + 'System ID: ' + skmsid + #0A +
/          'Version: ' + %editc(skmver:'X') + #0A;
/        if skmmdl <> ' ' and skmver < 902;
/          mailto = %trim(mailto) + 'Model: ' + skmmdl + #0A;
/        endif;
/        if skmftr <> ' ' and skmver < 902;
/          mailto = %trim(mailto) + 'Feature Code: ' + skmftr + #0A;
/        endif;
T6896    if skmlpr > 0 and skmsid <> 'DEMO';
/          mailto = %trim(mailto) + 'LPAR Number: ' +
J1299        %triml(%editc(skmlpr:'Z')) + #0A;
T6986    endif;
J1299    if skmlib <> ' ' and skmsid <> 'DEMO';
/          mailto = %trim(mailto) + 'Program Library: ' +  %trimr(skmlib);
/        endif;
T5671    mailto = %trim(mailto) + #0A;
/        mailto = %trim(mailto) + 'Product Name' + #09 + #09 + #09 + #09 +
/          'User License' + #09 + #09 + 'Expiration Date' + #0A;
         setll mst_key spykeylic;
         reade mst_key spykeylic;
         dow not %eof;
...
T5671      mailto = %trim(mailto) + getDsc + #09;
           if sklprd = 12;
           ...
           apnd('</td>');
T5671      mailto = %trim(mailto) + #09;
           if skl#us > 0;
             apnd(getElem('hUserBased'):lf);
             apnd(getElem('userBased'):%trim(%editc(sklprd:'Z')):
               ' value=':%trim(%editc(SKL#US:'Z')):
               '></td>');
T5671        mailto = %trim(mailto) + %editc(skl#us:'3');
           else;
             apnd(getElem('!userBased'));
           endif;
           mailto = %trim(mailto) + #09 + #09 + #09;
           if skmver >= 700 or skmpfm <> '400';
T5726        apnd(getElem('demoDate'):%trim(%editc(sklprd:'Z')):' value=':
/              %editw(skldmo:'    /  /  '):'></td>':lf);
T5671        mailto = %trim(mailto) + %editw(skldmo:'    /  /  ');
           endif;
           apnd('</tr>');
           mailto = %trim(mailto) + #0A;
T5671      if sklprd = 12;
/            mailto = %trim(mailto) + #09 + 'Server: ' + %trim(sktsvr) +
/              #09 + 'Key: ' + sktkcd + #0A;
/          endif;
           reade mst_key spykeylic;
         enddo;
           ...
           if skmver >= skpavf and skmver <= skpavt;
             ...
             if not %equal;
               ...
               if skputl = 'Y';
                 ...
                 apnd(getElem('!userBased'):lf);
               endif;
               if skmver >= 700 or skmpfm <> '400';
                 apnd(getElem('demoDate'):%trim(%editc(sklprd:'Z')):
                   '></td></tr>':lf);
               endif;
             endif;
           endif;
         ...
         apnd('</table>':lf);
         apnd('<br><table><tr><td>':getElem('checkAll'):'</td>':lf);
       ...
       begsr stdout_sr;
         ...
         apnd('</table></body></form></div></html>':lf);
     ...
     d apnd            pi
     d  val01                       100    value
     d  val02                       100    options(*nopass) value
     d  val03                       100    options(*nopass) value
     d  val04                       100    options(*nopass) value
     d  val05                       100    options(*nopass) value
     d  val06                       100    options(*nopass) value
     d  val07                       100    options(*nopass) value
     d  val08                       100    options(*nopass) value
     d  val09                       100    options(*nopass) value
     d  val10                       100    options(*nopass) value
     d  val11                       100    options(*nopass) value
     d  val12                       100    options(*nopass) value
     d  val13                       100    options(*nopass) value
...
<html>
<head><title>Open Text Key Code Maintenance</title>
<script language="javascript" src="keycode.js"></script></head>
<div align=center>
<form name=keycode method="post" action="keycodeweb">
<body topmargin=0 leftmargin=0 marginwidth=0 marginheight=0">
<img src=opentext.jpg>
<h2>Key Code Maintenance</h2>
**ctdata prdhdr
<table cellpadding=5><caption><b>Product List</b></caption>
<b><th>Select</th><th>Product Name</th><th>Number of Users</th>
<th>Expiration</th></b>
**ctdata envNamArr
SERVER_NAME
**ctdata chkbox
...
sltInstCap<table cellpadding=5><caption><b>Select Installation</b></caption>
sltInstTtl<tr><td></td><td>Customer Name</td><td>SystemID</td><td>PlatForm</td><td>Version</td></tr>
...
hUserBased<input type=hidden name=userbased value=1>
!userBased<input type=hidden name=userbased value=0><td>Not user based.</td>
