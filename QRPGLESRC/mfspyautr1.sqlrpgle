...
/          exsr unlock_us;
      ...
      //*************************************************************************
       BEGSR CHKUSRLIC;
         ...
         // Make sure the zero record exists.
         // If it does not exist, throw error. Record is added by the keycode
         // interface client mfspykeyr.
         exec sql select * into :licZero from mfult where rrn(mfult) = 1;
         // Not found, file has been tampered with.
         if sqlcod <> 0;
         endif;

         // Fetch this job's internal id.
         reset rjircv;
         reset apier;
         rji(rjircv:%size(rjircv):'JOBI0100':'*':' ':apier);

         // Determine if this job is in the licence tracking file.
         reset rjircv;
         exec sql select skuiji, skuhsh into :licRcvr from mfkyult
           where skuiji = :rjircv.rjiiji;

         // Job found in tracking file. Validate hash.
         if sqlcod = 0;
           pData = rjircv.rjiiji;
           MD5HASHA(pKey:%len(%trim(pKey)):PDATA:%LEN(%TRIM(PDATA)):tsthsh);
           if tsthsh <> licRcvr.hash;
             AR_IND = '0';
           endif;
           system('dltovr
           leavesr;
...
/8474      JOBID_PTR = usP;
      ...
      //*************************************************************************
       BEGSR LOCK_US;
         ...
         // Serialize access to license tracking file. Max wait 60 seconds.
...
/8130        rtvlckmsg(ultspc:ar_mdta);
           ...
           AR_IND = '0';
         ELSE;
J6082      ultFile = 'MFULT' + %editc(skl#us:'X') + 'U';
J6082      system('ovrdbf mfult ' + ultFile + ' ovrscope(*job)');
...
**CTDATA ALCDLC
ALCOBJ OBJ((*LIBL/MFULTXXXU *FILE *EXCL)) WAIT(60)
DLCOBJ OBJ((*LIBL/MFULTXXXU *FILE *EXCL))
