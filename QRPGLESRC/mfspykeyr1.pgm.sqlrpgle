     ...
     h copyright('Open Text Corporation')
      ...
      //
J6082 // 02-05-15 PLR Increased size of user tracking field from 3 to 6 positions.
      //              Doing so exceeds the max tracking capacity of user spaces.
      //              Converting to a slightly different methodology using
      //              files.
...
J6082d  rjierr                      116    options(*varsize:*nopass)
J6082d                 ds
J6082d rjircv                        61
...
J2163d setFldVal       pr
     ...
     D SPM             PR
     D  MSGID                         7    VALUE
J6082D  msgDta                       80    value options(*nopass)
     D STACKCNT        S             10I 0
...
J1299d  skmlib               296    305                                         Program Library
     D  DTALIB               306    315
      ...
      // LICENSE TRACKING USER SPACE.
     D                 DS
     D ULTSPC                        20    INZ('MFULTXXXU')
     D  ULTPRD#                       3    OVERLAY(ULTSPC:6)
     D  ULTSPCLIB                    10    OVERLAY(ULTSPC:11)
J6082D ultFile         s             21
...
J3754c                   parm                    opcode           10
      ...
      /FREE
J6082  exec sql set option closqlcsr=*endmod,commit=*none,srtseq=*langidshr;
...
/              endif;
/            endif;
J6082        if prodwork(i).skl#us > 0;
J2163          exsr crtUltSpc;
/            endif;
J6082        if errors;
J6082          leave;
J6082        endif;
...
J2163    loadWrkArr();
       ...
       BEGSR CRTULTSPC;
         ...
         // Create license tracking files.

J6082    ultprd# = %editc(sklprd:'X');
J6082    ultFile = %trimr(ultspclib) + '/' + %subst(ultspc:1:10);

J6082    // Delete the tracking file for the product.
J6082    sqlStmt = 'drop table ' + %trimr(ultFile);
J6082    exec sql execute immediate :sqlStmt;
J6082    if sqlcod < 0 and sqlcod <> -204; // -204 = not found.
J6082      spm('KY00062');
J6082      errors = *on;
J6082      leavesr;
J6082    endif;

J6082    sqlStmt = 'CREATE TABLE ' + %trimr(ultFile) + ' (' +
J6082      'SKUIJI CHAR (16) NOT NULL WITH DEFAULT,' +
J6082      'SKUHSH CHAR (16) NOT NULL WITH DEFAULT,' +
J6082      'PRIMARY KEY (SKUIJI))';
J6082    exec sql execute immediate :sqlStmt;
J6082    if sqlcod < 0;
J6082      spm('KY00062');
J6082      errors = *on;
...
J6082    // Create "zero" record to hold initial hash value for tracking file.

J6082    // Retrieve this job's internal job id.
J6082    clear rjircv;
J6082    reset apierr;
J6082    rji(rjircv:%size(rjircv):'JOBI0100':'*':' ':apierr);

J6082    pData = rjiiji;
J6082    MD5HASH(pKey:%len(%trim(pKey)):pData:%len(%trim(pData)):ulthsh);

J6082    sqlStmt = 'insert into ' + %trimr(ultFile) +
J6082      ' values(' + sq + rjiiji + sq + ',' + sq + ultHsh + sq + ')';
J6082    exec sql execute immediate :sqlStmt;
      ...
      /FREE
             ...
             WHEN CURRCD = 'WDWSFL01';
               ...
               *IN68 = NOT %FOUND;
               ...
               when csrfld = 'OPTION';
                 OPTION = SAVOPT;
               when skputl = 'Y' and csrfld = 'SKL#US';
                 SKL#US = save#us;
               when csrfld = 'SKLDMO';
                 SKLDMO = saveDemo;
               endsl;
...
/      EndSr;
     ...
     D SPM             PI
     D  MSGID                         7    VALUE
     D  msgDta                       80    value options(*nopass)
...
J6082 /free
J6082  if %parms > 1;
J6082    msgdtalen = %len(%trim(msgdta));
J6082  endif;
J6082 /end-free
