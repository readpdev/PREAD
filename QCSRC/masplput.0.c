...
static int fixSplSpc(SPOOL_HANDLE_T *spoolHandle); /*T6705*/
...
int putSplRcd(int hSplf, unsigned char *pSplRcd)
{
  ...
  fix1stBuf = 0;
  if (pSplRcdP->nPageOfs == 0xFFFF && pSPFRB->Buffer_Number == 1)
    fix1stBuf = 1;
  ...
  while(pSplRcdP->nPageOfs != 0xFFFF || fix1stBuf)  /*T6705*/
  {
    ...
    {
      fix1stBuf = 0;
      pSPFRP->Text_Data_Start = 1;
      pSPFRP->Any_Data_Start = 1;
      pSPFRP->Page_Offset = 0;
      pSplRcdP--;
...
static int putSplfDta(int hSplf)
{
  ...
  pApiErrCd = malloc(sizeof(*pApiErrCd));
  memset(pApiErrCd, 0, sizeof(*pApiErrCd));
  pApiErrCd->Bytes_Provided = sizeof(*pApiErrCd);

  QSPPUTSP(aSplfHdl[hSplf]->hOpnSplf, aSplfHdl[hSplf]->szUsQName, pApiErrCd);
  if (pApiErrCd->Bytes_Available > 0)                                             /*6606*/
  {                                                                               /*6606*/
    if (!memcmp(pApiErrCd->Exception_Id, "CPF33F2", 7) ||                         /*6705*/
       !memcmp(pApiErrCd->Exception_Id, "CPF33F7", 7))                            /*6705*/
    {                                                                             /*6705*/
      if (fixSplSpc(aSplfHdl[hSplf]) == SPL_OK)                                   /*6705*/
      {                                                                           /*6705*/
        memset(pApiErrCd, 0, sizeof(*pApiErrCd));                                 /*6705*/
        pApiErrCd->Bytes_Provided = sizeof(*pApiErrCd);                           /*6705*/
        QSPPUTSP(aSplfHdl[hSplf]->hOpnSplf, aSplfHdl[hSplf]->szUsQName,           /*6705*/
          pApiErrCd);                                                             /*6705*/
      }                                                                           /*6705*/
    }                                                                             /*6705*/
    if (pApiErrCd->Bytes_Available > 0)                                           /*6606*/
      return SPL_ERR_SPLF_PUT;                                                    /*6606*/
  }                                                                               /*6606*/
...
/* Correct the continuation flag in the general information buffer of the     */
/* user space. T6705                                                          */
static int fixSplSpc(SPOOL_HANDLE_T *spoolHandle)
{
  Qsp_SPFRH_t *pSPFRH;
  Qsp_SPFRB_t *pSPFRB;
  Qsp_SPFRG_t *pSPFRG;
  ...
  pSPFRH = spoolHandle->pBlkHdr;

  /* Walk the page buffers and update the continuation flags. Last flag = 'N'. */
  if (pSPFRH->Buffers_Returned > 1)
  {
    pSPFRB = ADDPTRO(pSPFRH, pSPFRH->Offset_First_Buffer);
    for (i=0;i<pSPFRH->Buffers_Returned;i++)
    {
      pSPFRG = ADDPTRO(pSPFRB, sizeof(*pSPFRB));
      SETFLAG(*pSPFRG->Last_Page_Continues, 1);
      if (pSPFRB->Buffer_Number == pSPFRH->Buffers_Returned)
        SETFLAG(*pSPFRG->Last_Page_Continues, 0);
      pSPFRB = ADDPTRO(pSPFRB, pSPFRB->Length_Buffer_Info);
    }
  }
