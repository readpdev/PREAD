       ...
       //  fetch the corresponding attributes from MRPTATR, clear the user
       //  defined data of the fileID information and update the attribute
       //  record (delete/write).
       //
...
/    d  pPath                          *   value options(*string)
     ...
     D*                                        Varying length
       ...
       dcl-ds printDS len(132) end-ds;
       dcl-c sq '''';

       dcl-pr system extproc('system');
         cmd pointer value options(*string:*trim);
       end-pr;
     ...
     D  SQL_00129            128    128A                                        end of header
       //*  exec sql set option closqlcsr=*endmod,commit=*none;
         ...
         system('ovrprtf qprint splfname(BPCLRPDFID)');
         open qprint;
         system('dltovr qprint');

         printIt('Balboa Peaks: Clear PDF FileID Utility');
         printIt('Exception messages only. Total updates are listed at ' +
           'bottom of report.');
         printIt(%char(%timestamp():*iso));
     ...
     C                   END                                                    SQL
         dow sqlcod = 0;
         clear qusa0200;
           // 4th parm 1=MRPTATR location.
           // qusud00 is where the fileID is stored.
           if getArcAtr(repind:%addr(qusa0200):%size(qusa0200):1) = 0;
             if qusudd00 <> ' '; // If there is data in usrdfndta, continue.
               if delArcAtr(repind) = 0; // Remove the mrptatr record.
                 clear qusudd00; // Clear the file id info.
                 // Write the updated mrptatr record.
                 if putArcAtr(repind:%addr(qusa0200)) = 0;
                   attrsUpdated += 1;
                 else;
                   printIt(repind + ' MRPTATR: Error on update.');
                 endif;
               else;
                 printIt(repind + ' MRPTATR: Error on delete.');
               endif;
             endif;
           else;
             printIt(repind + ' MRPTATR: Record not found.');
           endif;
       //*    exec sql fetch c1 into :mrptdir;
     ...
     C                   END                                                    SQL
         ...
         printIt(%editc(attrsUpdated:'1') + ' Attributes Updated');
