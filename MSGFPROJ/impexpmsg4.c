...
char qualMsgf[21], qualOrigEngMsgf[21];
char syscmd[256];

void main(int argc, char *argv[])
{
  char msgfLib[11], msgf[11], path[257], origEngMsgf[11], origEngMsgfLib[11];
  ...
  memset(&origEngMsgfLib, 0, sizeof(msgfLib));
  memset(&origEngMsgf, 0, sizeof(msgf));
  ...
  if (operFlag == IMPORT)
  {
    memcpy(&origEngMsgf, argv[7], sizeof(origEngMsgf));
    memcpy(&origEngMsgfLib, argv[8], sizeof(origEngMsgfLib));
  }
  ...
  while (fgets(buf, MAXBUF-1, fh))
  {
    ...
    if ((rc = system(syscmd)))
    {
      if (strlen(msg.lvlOne) > MAX_LVL_ONE_LEN)
        ...
        /* message may not exist, try adding */
        memset(&errorCode, 0, sizeof(errorCode));
        errorCode.error.Bytes_Provided = sizeof(errorCode);
        if ((rc = msgAPI(&rcvr, msg.msgID, qualMsgf, " ", 0, " ", rtvOpt)))
        {
          rc = -1;
          if (rc == 1) /*end of msgf*/
            rc = 0;
        ...
        /* This is just a temp workaround until there's time to do this the right way. */
